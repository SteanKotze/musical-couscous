class CustomGrid {
    constructor() {
        ...
        this.config = {
            ...
            events: {
                tbodyTr_onClick: null
            },
            styles: {
                container: "", table: "", 
                thead: "", theadTr: "", theadTrTh: "",
                tbody: "", tbodyTr: "", tbodyTrTd: "",
                tbodyTrOdd: "", tbodyTrEven: "",
                tfoot: "", tfootTr: "", tfootTrTd: "",
                paginationInfoContainer: "", paginationBtnsContainer: "", 
                paginationBtn: "", paginationBtnStandard: "", paginationBtnInactive: "", paginationBtnActive: ""
            },
            logLevel: "error"
        }
            ...
            .map((x, i) => {
                ...
                return {
                    ...
                    hidden: configColumn !== undefined && configColumn.hidden !== undefined
                        ? configColumn.hidden
                        : lowerX !== "id" && lowerX.endsWith("id") || configColumn === undefined,
                    width: configColumn !== undefined && configColumn.width !== undefined
                        ? configColumn.width
                        : null,
                    ...
                    onClick: configColumn !== undefined && configColumn.onClick !== undefined
                        ...
                        : null,
                    style: configColumn !== undefined && configColumn.style !== undefined
                        ? configColumn.style
                        : null,
                    mapping: configColumn !== undefined && configColumn.mapping !== undefined
                        ? configColumn.mapping
                        : null
                }
        ...
        gridHtml += this.config.columns
            ...
            .map(x => {
                ...
                if (x.key === "actions" && this.config.actions !== null)
                    return `<th class='${this.config.styles.theadTrTh} ${x.style} ${this.config.styles.actions}' ${colWidth}>${x.header}</th>`
                return `<th class='${this.config.styles.theadTrTh} ${x.style ?? ''}' ${colWidth}>${x.header}</th>`
            })
            .join('')
        ...
        $(`#${this.name}_tableBody`).html(
            data.map((x, i) => {
                ...
                var keyHtml = key !== undefined && key !== null
                    ? `entity-id='${key}'`
                    : ""
                return `<tr class='${this.config.styles.tbodyTr} ${i%2 == 0 ? this.config.styles.tbodyTrEven : this.config.styles.tbodyTrOdd}'
                    ${keyHtml}
                    ${this.config.events.tbodyTr_onClick ? `onclick="${this.config.events.tbodyTr_onClick}('${key}')"` : ''}>
                    ${
                        this.config.columns
                            .filter(y => !y.hidden)
                            .map(y => {
                                if (y.key === "actions" && this.config.actions !== null) {
                                    return `<td class='${this.config.styles.actions} ${y.style ?? ''}'>${this.config.actions.map(z => {
                                ...
                                var cellValue = x[y.key]
                                if (y.mapping && cellValue !== undefined && cellValue !== null)
                                    cellValue = y.mapping(cellValue)

                                return `<td class='${this.config.styles.tbodyTrTd} ${y.style ?? ''} ${y.onClick ? 'cursor-pointer hover-underline' : ''}'
    ...
    renderPaginationButtons() {
        ...
        var paginationHtml = `<div class="${this.config.styles.paginationBtnsContainer}">`
        var paginationInfoHtml = `<div class="${this.config.styles.paginationInfoContainer}">`
        ...
        if (this.currentPageIndex === 0) {
            ...
            paginationHtml += `
                <button class='rounded-start ${this.config.styles.paginationBtn} ${this.config.styles.paginationBtnStandard}' onclick='${this.name}.pageTo(0)'>
                    <<
                </button>
                <button class='${this.config.styles.paginationBtn} ${this.config.styles.paginationBtnStandard}' onclick='${this.name}.pageTo(${this.currentPageIndex - 1})'>
        ...
        if (this.totalPages > this.currentPageIndex + 1) {
            paginationHtml += `
                ...
                <button class='rounded-end ${this.config.styles.paginationBtn} ${this.config.styles.paginationBtnStandard}' onclick='${this.name}.pageTo(${this.totalPages - 1})'>
                    >>
                </button>`
        } else {
            paginationHtml += `
        ...
        paginationHtml += "</div>"
        paginationInfoHtml += `Page ${this.currentPageIndex + 1} of ${this.totalPages} (${this.pages[0].data.count} items)</div>`
